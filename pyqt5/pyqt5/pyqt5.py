...
if((if_6_flag==False)and(array_answer[0]==0)and(((self.path[array_answer[1].n1].number!=self.start_node)and(len(array_answer)==2))
or((len(array_answer)>2)and(self.path[array_answer[len(array_answer)-1].n1].number!=self.start_node)))
and(self.count_passed_now_mesh==0)and(len(self.mesh_arr)>0)): #
